...
TEST(BEElementRefDecode, ZeroString_IsValid)
{
	...
	ASSERT_TRUE(decoded);
	ASSERT_EQ(nonstd::string_view(), *decoded);
}
...
TEST(BEElementRefDecode, String_Is_NonNegative_Number_Sepatated_With_Colon)
{
	...
	ASSERT_TRUE(decoded);
	ASSERT_EQ(nonstd::string_view("str"), *decoded);
}
...
{
	{
		...
		ASSERT_TRUE(decoded);
		ASSERT_EQ(nonstd::string_view("0"), *decoded);
	}
...
TEST(BEElementRefDecode, Only_Negative_Integers_Parsed)
{
	...
	for (const char* integer : k_integers)
	{
		...
		ASSERT_TRUE(decoded);
		ASSERT_EQ(nonstd::string_view(integer), *decoded);
	}
}
